    ...
    public void addNewWorker(Worker newWorker, double salary) {
        try {
            if (isPersonBelongsToStaff(newWorker)) {
                throw new PersonBelongsToWorkersException("This person is already in staff!");
            }
        ...
        } catch (PersonBelongsToWorkersException | WrongSalaryException exception) {
    ...
    public void deleteWorker(Worker firedWorker) {
        try {
            if (!isPersonBelongsToStaff(firedWorker)) {
                throw new PersonBelongsToWorkersException("This person doesn't belong to staff!");
            }
        ...
        } catch (PersonBelongsToWorkersException exception) {
            System.out.println(exception.getMessage());
        }
    }
